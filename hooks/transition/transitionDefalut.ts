...
export function tdStart(transitionPage: transitionPage): [HTMLDivElement, gsap.core.Timeline] {
  const svg =
    direction => `<svg class="dsn-separator-${direction} dsn-icon-assistant-color" width="100%" height="100%" viewBox="0 0 100 10"
  ...
  domTransition.innerHTML = `
            ...
            <h6 class="d-flex justify-content-center title text-uppercase">${
              typeof transitionPage === 'object' ? transitionPage?.title : 's.ludwikowski'
            }</h6>
            ${svg('bottom')}
            `
